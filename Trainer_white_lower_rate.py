...
def main ():
    ...
    player1 = DQN_Agent(player=1, env=env,parametes_path='')
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch+1) % 100 == 0:
            ...
            if best_res < res:      
                best_res = res
                if best_res > 75 and tester_fix(1) == (1,0,0):
        ...
        if (epoch+1) % 1000 == 0:
            ...
            if best_random < test_score and tester_fix(1) == (1,0,0):
                best_random = test_score
                player1.save_param(path_best_random)
            print(test)
            requests.post("https://ntfy.sh/dqnhnefatafltraining2", data=f'In epoch {epoch+1}, test results: {test}, averageloss: {avgLoss}'.encode(encoding='utf-8'))
