...
def main ():
    ...
    player1 = DQN_Agent(player=Player.DEFENDER, env=env,parametes_path=path_load)
    player_hat = DQN_Agent(player=Player.DEFENDER, env=env, train=False)
    Q = player1.DQN
    Q_hat = Q.copy()
    Q_hat.train = False
    ...
    player2 = Random_Agent(player=Player.ATTACKER, env=env)  
    ...
    best_res = -200
    loss_count = 0
    tester = Tester(player1=Random_Agent(player=Player.ATTACKER, env=env), player2=player1, env=env)
